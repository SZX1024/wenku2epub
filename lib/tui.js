...
const { sanitizeFilename, getOutputDir, parseIndexSpec, formatIndexLabel } = require('./config');
...
function reportCache() {
  ...
  if (stats.hits > 0 || stats.writes > 0) {
    console.log(`💾 缓存命中 ${stats.hits} 次，新下载 ${stats.writes} 次（${cache.CACHE_ROOT}）`);
  }
}
...
async function main() {
  ...
  try {
    ...
    const json = {};
    await getBookInfo(url.trim(), json);
    ...
    setConcurrency({
      chapters: Number(concurrencyChoice),
      images: [1, 2, 3, 5, 8].includes(Number(config.imageConcurrency)) ? Number(config.imageConcurrency) : 3
    });
