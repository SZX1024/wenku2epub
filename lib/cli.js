...
const OPTION_SPEC = {
  ...
  noIllustrationAnchors: { flags: ['--no-illustration-anchors'], help: '不按（插图NNN）标记把插图放回正文（默认开启）' },
...
function resolveOptions(argv) {
  ...
  return {
    ...
    // 按（插图NNN）标记把插图放回正文；同样默认开启
    illustrationAnchors: provided.has('noIllustrationAnchors')
      ? false
      : toBool(config.illustrationAnchors ?? true, '配置项 illustrationAnchors'),
...
async function runOne(options, url, run) {
  ...
  console.log(`⚙️  延迟：${options.delay ? '开' : '关'}｜章节并发：${options.concurrency}｜插图并发：${options.imageConcurrency}｜缓存：${options.cache ? (options.refresh ? '刷新' : '开') : '关'}｜去重：${options.dedupeImages ? '开' : '关'}${options.compressImages ? '｜压缩：开' : ''}｜插图回填：${options.illustrationAnchors ? '开' : '关'}\n`);
  ...
  const built = await scraper({
    ...
    illustrationAnchors: options.illustrationAnchors,
    run,
  });
