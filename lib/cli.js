...
const { setDelayMode, setRateLimit } = require('./fetch');
const { setConcurrency } = require('./epub');
const {
  setOutputDir, getOutputDir, sanitizeFilename, parseIndexSpec, formatIndexLabel,
} = require('./config');
...
async function runOne(options, url) {
  ...
  console.log(`💾 输出：${path.resolve(getOutputDir())}`);
  ...
  if (options.format === 'txt') {
    ...
    const { failedChapters } = await exportAsTxt(json, selectedVolumes, { chapterSelection });
...
async function runCli(options) {
  ...
  cache.configure({ enabled: options.cache, refresh: options.refresh });

  if (options.clearCache) {
    cache.clear();
    console.log(`✅ 缓存已清空：${cache.CACHE_ROOT}`);
    return 0;
  }
  if (options.cacheInfo) {
    const info = cache.size();
    console.log(`缓存目录：${info.root}`);
    console.log(`文件数：${info.files}，占用：${cache.formatSize(info.bytes)}`);
    return 0;
  }
  ...
  setOutputDir(options.out);
  setDelayMode(options.delay);
  if (options.rate !== null) setRateLimit(options.rate);
  setConcurrency({ chapters: options.concurrency, images: options.imageConcurrency });
  ...
  for (let i = 0; i < options.urls.length; i++) {
    ...
    try {
      const code = await runOne(options, url);
