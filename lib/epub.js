...
// 彩插页：放置该卷编号 1..k-1 的图（口绘/角色页之类），排在卷首页之前。
...
function buildColorPage(imgNames, epubVersion) {
  ...
  const body = imgNames
    ...
    .join('\n');
  return xhtmlShell({ title: '彩页', epubVersion, body, extraStyle: style });
}
...
// 把一卷里各章的图片槽位按章节顺序拼起来，得到"卷内第 N 张图"的索引。
// slots 与文档顺序严格对齐（下载失败的记为 null），所以失败不会让后面的序号错位。
function buildVolumeImagePlan(json, volIdx) {
  const plan = [];
...
// 回填单章：把占位段落按顺序换成真正的 <img>，换不了的就保留原始标记文字。
async function fillChapterMarkers(book, json, volIdx, chapterIdx, numbers, plan, warnings) {
  ...
  for (let i = 0; i < Math.min(placeholders.length, numbers.length); i++) {
    const no = numbers[i];
    const imgname = no >= 1 && no <= plan.length ? plan[no - 1] : null;

    if (!imgname) {
      const why = no < 1 || no > plan.length
        ? `编号 ${no} 超出本卷图片总数 ${plan.length}`
        : `第 ${no} 张图下载失败`;
      warnings.push(`⚠️  ${volumeName} / ${chapterTitle}：（插图${no}）${why}，已保留原始标记文字`);
      continue;
    }
    ...
    placeholders[i].parentNode.replaceChild(img, placeholders[i]);
    filled++;
  }
...
// 回填全部卷：把有标记的图放到正文对应位置，并整理出每卷的彩插清单。
// 没有标记的卷完全不处理 —— 插图仍然留在最后的「插图」章里。
async function applyIllustrationAnchors(book, json, selectedVolumes = null, logger = console) {
  const warnings = [];
  json._colorPages = {};   // { volIdx: [imgname, ...] } 卷首彩插
  ...
  for (const volIdx of writtenVolumes(json, selectedVolumes)) {
    ...
    const marked = [];
    for (const chapter of chapters) {
      for (const no of json._markers?.[volIdx]?.[chapter] || []) marked.push(no);
    }
    if (marked.length === 0) continue;

    const plan = buildVolumeImagePlan(json, volIdx);

    // 只用"编号有效且图片确实拿到"的标记来决定切分点。
    // 否则一个越界标记（如 099）会把 1..98 全判成彩插，等于把整卷插图搬到卷首。
    const usable = marked.filter(no => no >= 1 && no <= plan.length && plan[no - 1]);
    if (usable.length === 0) continue;

    // 编号小于最小可用标记的那些图没有位置信息，按彩插处理放在卷首
    const firstMarked = Math.min(...usable);
    const colorImages = plan.slice(0, Math.max(0, firstMarked - 1)).filter(Boolean);
    if (colorImages.length > 0) json._colorPages[volIdx] = colorImages;

    let filled = 0;
    for (const chapter of chapters) {
      const numbers = json._markers?.[volIdx]?.[chapter] || [];
      if (numbers.length === 0) continue;
      filled += await fillChapterMarkers(book, json, volIdx, chapter, numbers, plan, warnings);
    }

    if (filled > 0 || colorImages.length > 0) {
      logger.info(`📑 ${json.content[volIdx]?.volume ?? volIdx}：${filled} 张插图已放回正文，${colorImages.length} 张彩插置于卷首`);
    }
  }
