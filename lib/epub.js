...
// 彩插页：放置卷内"没有位置信息"的图，排在卷首页之前。
function colorPageName(volume) {
  return `color_${volume}.xhtml`;
}

// 卷尾页：放置最后一个已映射图之后的未映射图，排在该卷最后一章之后。
function endPageName(volume) {
  return `color_end_${volume}.xhtml`;
}

// 正文里插图标记的占位段落所使用的 class。
// 不用 data-* 属性是因为 EPUB2 基于 XHTML 1.1，不允许自定义 data-* 属性。
const MARKER_CLASS = 'ill-pending';

function buildImagePage(imgNames, title, epubVersion) {
  const style = `    html, body { margin: 0; padding: 0; }
    .colorpage { text-align: center; margin: 0; padding: 0; }
    .colorpage img { max-width: 100%; }`;
  const body = imgNames
    .map(name => `  <div class="colorpage">\n    <img src="../Image/${name}" alt="" />\n  </div>`)
    .join('\n');
  return xhtmlShell({ title, epubVersion, body, extraStyle: style });
}

function buildColorPage(imgNames, epubVersion) {
  return buildImagePage(imgNames, '彩页', epubVersion);
}

function buildEndPage(imgNames, epubVersion) {
  return buildImagePage(imgNames, '卷尾插图', epubVersion);
}
...
async function processChapter(ctx, volume, chapter, title, href) {
  const { book, json, parser, serializer, progress, epubVersion, imgCounter, run } = ctx;

  const $ = await run.net.loadChapterPage(href);
  ...
  const pending = [];
  const markers = [];   // 本章出现的插图标记编号，按文档顺序
  for (const item of extractChapterContent($, content)) {
    ...
    if (item.type === 'marker') {
      if (!ctx.illustrationAnchors) {
        // 功能关闭时标记就是普通文字，行为与改动前一致
        const p = xhtml.createElement('p');
        p.textContent = item.raw;
        container.appendChild(p);
        continue;
      }
      // 先写成占位段落，等所有章节下载完、卷内图片序号确定后再回填。
      // 占位里保留原文：万一回填失败，读者看到的还是原始的（插图NNN），不会丢内容。
      const p = xhtml.createElement('p');
      p.setAttribute('class', MARKER_CLASS);
      p.textContent = item.raw;
      container.appendChild(p);
      markers.push(item.no);
      continue;
    }
  ...
  const results = await Promise.all(pending.map(entry => run.limits.images(async () => {
    const raw = await run.net.loadImage(entry.absSrc);
    if (!raw) return { entry, ok: false };

    let data = raw;
    if (ctx.compressImages) {
      const compressed = await compressImage(raw, ctx.imageQuality, run.logger);
  ...
  // slots 与本章图片的文档顺序严格对齐（失败的记为 null），
  // 这样卷内"第 N 张图"的序号在遇到下载失败时也不会错位。
  const slots = [];
  ...
  return { ok: true, images: slots, markers };
}
...
async function creatText(book, json, selectedVolumes = null, progress = null, epubVersion = 3, {
  ...
  json._slots = {};     // { volIdx: { chapterIdx: [imgname|null] } } 与文档顺序严格对齐
  json._markers = {};   // { volIdx: { chapterIdx: [编号] } } 与文档顺序严格对齐
  json._colorPages = {}; // { volIdx: [imgname, ...] } 卷首彩插，未启用索引时保持为空

  if (compressImages) warnSharpMissing(run.logger);
  ...
  for (const volume in json.content) {
    ...
    for (const chapter in json.content[volume].chapters) {
      ...
      tasks.push(run.limits.chapters(async () => {
        const result = await processChapter(ctx, volIdx, chapterIdx, title, href);
        if (result.ok) {
          json._written[volIdx].push(chapterIdx);
          json._volImgs[volIdx].push(...result.images.filter(Boolean));
          json._slots[volIdx][chapterIdx] = result.images;
          json._markers[volIdx][chapterIdx] = result.markers;
        } else {
  ...
  // 回填必须放在最后：此时"卷内第 N 张图"才有确定的文件名
  if (illustrationAnchors) {
    await applyIllustrationAnchors(book, json, selectedVolumes, run.logger);
  }
}

// ── 插图索引回填 ──

// 画廊 = 标题含「插图/插画/彩页/扉页」的那一章里的图片。
// 标记编号是相对画廊的；后面的「特典」等章节里的图不参与编号
//（3396 第二卷的插图章 17 张、后面两个特典章各 1 张，按整卷算会整体错位两张）。
const GALLERY_TITLE = /插图|插画|插圖|彩页|彩图|扉页|口绘/;

function buildGalleryPlan(json, volIdx) {
  const chapters = Object.keys(json.content[volIdx]?.chapters || {})
    .map(Number)
    .sort((a, b) => a - b);
  const galleryChapter = chapters.find(ch => GALLERY_TITLE.test(json.content[volIdx].chapters[ch]?.title || ''));

  const used = galleryChapter === undefined ? chapters : [galleryChapter];
  const plan = [];
  for (const chapter of used) {
    for (const name of json._slots?.[volIdx]?.[chapter] || []) plan.push(name);
  }
  return { plan, hasGalleryChapter: galleryChapter !== undefined };
}
...
function describeNumbers(numbers) {
  ...
  const missing = [];
  for (let n = min; n <= max; n++) if (!sorted.includes(n)) missing.push(n);
  const range = min === max ? String(min) : `${min}~${max}`;
  return missing.length > 0 ? `${range}，缺 ${missing.join(',')}` : range;
}

// 回填单章：把占位段落按顺序换成图片组，换不了的就保留原始标记文字。
// groups 与本章标记顺序一一对应，每组第一张是该标记自己的图，
// 后面跟的是"编号缺口里的图"（就近跟随前一张已映射图）。
async function fillChapterMarkers(book, json, volIdx, chapterIdx, markers, groups, warnings) {
  const file = book.file(`OEBPS/Text/${chapterFileName(volIdx, chapterIdx)}`);
  if (!file) return 0;
  ...
  for (let i = 0; i < all.length; i++) {
    const el = all[i];
    if (el.getAttribute && el.getAttribute('class') === MARKER_CLASS) placeholders.push(el);
  }

  const volumeName = json.content[volIdx]?.volume ?? volIdx;
  const chapterTitle = json.content[volIdx]?.chapters?.[chapterIdx]?.title ?? chapterIdx;
  let filled = 0;

  for (let i = 0; i < Math.min(placeholders.length, groups.length); i++) {
    const group = groups[i];
    if (!group || group.length === 0) {
      const no = markers[i];
      warnings.push(`⚠️  ${volumeName} / ${chapterTitle}：标记 ${no === null ? '（无编号）' : no} 无法定位，已保留原始文字`);
      continue;
    }

    // 用一个容器包住这一组图，避免多张图散落
    const holder = doc.createElement('div');
    holder.setAttribute('class', 'illustration-group');
    for (const imgname of group) {
      const img = doc.createElement('img');
      img.setAttribute('src', `../Image/${imgname}`);
      img.setAttribute('alt', '');
      holder.appendChild(img);
    }
    placeholders[i].parentNode.replaceChild(holder, placeholders[i]);
    filled++;
  }

  if (filled > 0) {
    const formatted = beautify(new XMLSerializer().serializeToString(doc), { indent_size: 2 });
    book.file(`OEBPS/Text/${chapterFileName(volIdx, chapterIdx)}`, Buffer.from(iconv.encode(formatted, 'utf-8')));
  }
  return filled;
}
...
// 回填全部卷。
//
// 映射规则（在两本书 5 个卷上实测成立，并用像素饱和度独立复核）：
//     图序号 = 标记编号 + (画廊图数 − 最大标记编号)
// 即"被标记的图永远是画廊尾部那一段"——彩页在前、插画在后是轻小说的通行排版。
//
// 未映射的图按位置三分：
//   第一个已映射图之前 → 卷首彩页页
//   编号缺口里的      → 就近跟随前一张已映射图（和它放在同一个位置）
//   最后一个已映射图之后 → 卷尾页（按当前规则恒为空，保留以防规则变化）
//
// 「插图」章始终保留全部图片，不受这里影响。
async function applyIllustrationAnchors(book, json, selectedVolumes = null, logger = console) {
  const warnings = [];
  json._colorPages = {};   // { volIdx: [imgname] } 卷首
  json._endPages = {};     // { volIdx: [imgname] } 卷尾
  const report = [];

  for (const volIdx of writtenVolumes(json, selectedVolumes)) {
    const chapters = json._written[volIdx] || [];
    const volumeName = json.content[volIdx]?.volume ?? volIdx;

    // 本卷标记：按章、按文档顺序。no 可能为 null（只写了关键词没写编号）
    const markers = [];
    for (const chapter of chapters) {
      for (const no of json._markers?.[volIdx]?.[chapter] || []) markers.push({ chapter, no });
    }

    if (markers.length === 0) {
      report.push(`📑 ${volumeName}：未识别到插图标记，插图保持在「插图」章`);
      continue;
    }

    const numbered = markers.filter(m => m.no !== null).map(m => m.no);
    const { plan, hasGalleryChapter } = buildGalleryPlan(json, volIdx);

    // 编号必须能整体落在画廊范围内，且映射后严格递增
    const sorted = [...numbered].sort((a, b) => a - b);
    const offset = plan.length - Math.max(...sorted);
    const mapped = sorted.map(n => n + offset);
    const valid = numbered.length > 0
      && offset >= 0
      && mapped.every(g => g >= 1 && g <= plan.length && plan[g - 1])
      && mapped.every((g, i) => i === 0 || g > mapped[i - 1]);

    if (!valid) {
      report.push(`📑 ${volumeName}：识别到 ${markers.length} 个插图标记，但无法安全映射（画廊 ${plan.length} 张、编号 ${describeNumbers(numbered)}），已保留原始文字`);
      for (const m of markers) {
        warnings.push(`⚠️  ${volumeName} / ${json.content[volIdx]?.chapters?.[m.chapter]?.title ?? m.chapter}：标记 ${m.no ?? '（无编号）'} 未放置`);
      }
      continue;
    }

    // 每个编号 → 要插入的图片组（自己 + 紧跟其后的缺口图）
    const groupsByNo = new Map();
    for (let i = 0; i < sorted.length; i++) {
      const start = sorted[i] + offset;
      const end = i + 1 < sorted.length ? sorted[i + 1] + offset : plan.length + 1;
      const group = [];
      for (let g = start; g < end; g++) if (plan[g - 1]) group.push(plan[g - 1]);
      groupsByNo.set(sorted[i], group);
    }

    // 卷首：第一个已映射图之前的
    const frontImages = plan.slice(0, sorted[0] + offset - 1).filter(Boolean);
    if (frontImages.length > 0) json._colorPages[volIdx] = frontImages;

    // 卷尾：最后一个已映射图之后的
    const lastMapped = sorted[sorted.length - 1] + offset;
    const backImages = plan.slice(lastMapped).filter(Boolean);
    if (backImages.length > 0) json._endPages[volIdx] = backImages;

    let filled = 0;
    for (const chapter of chapters) {
      const numbers = json._markers?.[volIdx]?.[chapter] || [];
      if (numbers.length === 0) continue;
      const groups = numbers.map(no => (no === null ? null : groupsByNo.get(no) || null));
      filled += await fillChapterMarkers(book, json, volIdx, chapter, numbers, groups, warnings);
    }

    const gapCount = groupsByNo.size > 0
      ? [...groupsByNo.values()].reduce((sum, g) => sum + g.length, 0) - groupsByNo.size
      : 0;
    report.push(
      `📑 ${volumeName}：识别到 ${markers.length} 个插图标记 [${describeNumbers(numbered)}]`
      + ` → 映射到${hasGalleryChapter ? '画廊' : '全卷'}第 ${mapped[0]}~${mapped[mapped.length - 1]} 张（共 ${plan.length} 张）`
      + `｜卷首 ${frontImages.length} 张、缺口跟随 ${gapCount} 张、卷尾 ${backImages.length} 张`
      + `｜已放回正文 ${filled} 处`,
    );
  }
...
async function creatOpf(book, json, {
  ...
  for (const volume of writtenVolumes(json, selectedVolumes)) {
    // 彩插页排在该卷卷首页之前；第一卷时它紧跟在封面之后
    if ((json._colorPages?.[volume] || []).length > 0) {
      const colorId = itemId('color', volume);
      addItem(colorId, `Text/${colorPageName(volume)}`, 'application/xhtml+xml');
      addSpineRef(colorId);
    }
    ...
    // 卷尾页排在最后一章之后
    if ((json._endPages?.[volume] || []).length > 0) {
      const endId = itemId('colorend', volume);
      addItem(endId, `Text/${endPageName(volume)}`, 'application/xhtml+xml');
      addSpineRef(endId);
    }
  }
...
async function creatEpub(json, {
  ...
  // 4) 每卷生成卷首页；有彩插的卷再生成一个彩插页（排在卷首页之前）
  for (const volume of vols) {
    ...
    const colorImages = json._colorPages?.[volume] || [];
    if (colorImages.length > 0) {
      const colorPage = buildColorPage(colorImages, epubVersion);
      book.file(`OEBPS/Text/${colorPageName(volume)}`, Buffer.from(iconv.encode(colorPage, 'utf-8')));
    }

    const endImages = json._endPages?.[volume] || [];
    if (endImages.length > 0) {
      const endPage = buildEndPage(endImages, epubVersion);
      book.file(`OEBPS/Text/${endPageName(volume)}`, Buffer.from(iconv.encode(endPage, 'utf-8')));
    }
  }
