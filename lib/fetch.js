...
let rateLimitMs = 0; // 0 = 不限速

function setDelayMode(enabled) {
  delayOrNot = Boolean(enabled);
  rateLimitMs = delayOrNot ? BASE_DELAY_MS : 0;
}

// 显式设置全局限速间隔（毫秒），0 表示关闭
function setRateLimit(ms) {
  const value = Number(ms);
  rateLimitMs = Number.isFinite(value) && value > 0 ? value : 0;
  delayOrNot = rateLimitMs > 0;
}
...
// 而不是像以前那样"正文 sleep、图片不限速"——后者正是触发 429 的主要原因。
let gate = Promise.resolve();
let nextSlotAt = 0;

function acquireSlot() {
  if (rateLimitMs <= 0) return Promise.resolve();

  const wait = gate.then(async () => {
    const now = Date.now();
    const readyAt = Math.max(now, nextSlotAt + rateLimitMs + Math.floor(Math.random() * JITTER_MS));
    nextSlotAt = readyAt;
...
async function fetchWithRetry(url, maxRetries = 3, options = {}) {
  const data = await requestBuffer(url, htmlHeaders(), maxRetries, options);
  if (!data) return null;
...
async function askChapter(url) {
  return fetchWithRetry(url, 3, { cacheKind: 'html' });
}

// 下载插图，返回 Buffer（失败返回 null）。
// 由调用方决定怎么命名、是否去重、是否压缩、写进 zip 还是文件系统。
async function fetchImage(src, maxRetries = 3) {
  return requestBuffer(src, imageHeaders(new URL(src).origin + '/'), maxRetries, { cacheKind: 'img' });
}
