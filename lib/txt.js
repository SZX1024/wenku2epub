...
async function processChapterTxt(href, volIdx, chapIdx, run) {
...
async function exportAsTxt(json, selectedVolumes, {
  ...
  for (const volIdx of volumesToProcess) {
    ...
    for (const chapter in volData.chapters) {
      ...
      const result = await processChapterTxt(href, volIdx, chapterIdx, run);
      ...
      for (const img of result.images) {
        imgTasks.push(run.limits.images(async () => ({ name: img.name, data: await run.net.loadImage(img.src) })));
      }
