...
const { sanitizeFilename, getOutputDir, parseVolumeNumber } = require('./config');
...
function writeFailureReport(json, { url, safeTitle, skipped }) {
  ...
  const file = path.join(getOutputDir(), `${safeTitle}.failed.json`);
  try {
...
async function scraper({
  ...
  const imageOptions = { chapterSelection, dedupeImages, compressImages, imageQuality };
  ...
  if (saveMode === 'separate' && volumesToProcess.length > 0) {
    ...
    writeFailureReport(json, { url, safeTitle, skipped: allSkipped });
    return built;
  }
  ...
  reportSkipped(json);
  writeFailureReport(json, { url, safeTitle, skipped: json.skippedTotal });
