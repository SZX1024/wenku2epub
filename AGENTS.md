...
| `lib/fetch.js` | HTTP with global rate limiting, retry/backoff, caching, GBK decoding |
| `lib/cache.js` | Content-addressed on-disk cache for chapter HTML and images |
...
| `lib/config.js` | Constants, XML/XHTML templates, filename/numeral/index-spec/output-dir helpers |
...
3. **Fetching** (`lib/fetch.js` + `lib/cache.js`): a single `requestBuffer()` is the only HTTP entry point. It does cache lookup → in-flight de-duplication → global rate-limit slot → retry. **`ask()` never caches** (book/index pages change as chapters are published); **`askChapter()` and image downloads do** (published content is immutable). This is what makes re-running for new chapters an incremental update.
...
6. **Output**: JSZip archive written to `getOutputDir()` (default `output/`, overridable with `-o`). `scraper()` accepts an options object and can reuse an already-fetched `json` (both front ends pass it in) to avoid re-requesting the book page.

## EPUB correctness invariants

These were all previously violated and are now covered by `test/epub.test.js`:
...
- **Single HTTP entry point** (`lib/fetch.js`): `requestBuffer()` does cache → in-flight de-dup → rate-limit slot → retry. Add new requests through it, not with raw `axios`.
- **Caching** (`lib/cache.js`): content-addressed by URL under `.cache/wenku2epub/` (override with `WENKU2EPUB_CACHE_DIR`). Only immutable content is cached (`askChapter`, images). This makes "re-run to pick up new chapters" an incremental update.
- **Global rate limiting**: `setDelayMode(true)` installs a jittered 500–1000 ms minimum interval shared by *all* requests; `setRateLimit(ms)` sets it explicitly. Concurrency is `setConcurrency({ chapters, images })`, capped at 8.
...
- **Success signalling**: `fetchImage()` returns a `Buffer` or `null`, and callers only register artifacts that really landed (a failed image loses its `<img>` tag and never reaches the manifest).
- **User-Agent rotation**: `getRandomUserAgent()` returns one of ~10 desktop/mobile UAs.
- **Encoding**: wenku8 pages are GBK; `iconv-lite` decodes before cheerio parsing.
- **Paths**: `style.css` and local `cover.*` resolve against `PROJECT_ROOT`; the output root comes from `getOutputDir()` (`lib/config.js`). Never use `process.cwd()` directly.
